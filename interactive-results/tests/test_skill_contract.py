...
class SkillWorkflowTests(unittest.TestCase):
    def test_workflow_contract(self):
        ...
        for phrase in ('/interactive-results <run_dir>','path is mandatory','never guess','phase 8 completion','snapshot-only','must not perform fresh web lookup','06-decisions.json','interactive-results-migration.json','consequential_ambiguities','use your defaults','required facet-design checkpoint','practical','audit','08-results.md','interactive-results/generate.py','python3 -m http.server 8765','browser validation is mandatory','interactive-results-validation.json','update the manifest','separate explicit request','no automatic publication'):
