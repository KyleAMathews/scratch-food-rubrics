...
## 2026-07-21 — v8.14 full-population rescore migration

- **What happened:** Moved the frozen 1,650-record evidence run onto the main checkout and rescored it under the revised scratch contract. The 207 records with recoverable five-part legacy vectors were converted by discarding volatility and proportionally remapping the four retained prior judgments to the new 25/40/20/15 maxima. Another 444 records now receive non-normalized partial S scores because they have accepted current production evidence but incomplete criterion coverage. Arempas is now `34/40` on the observed production criterion with 40% coverage rather than unresolved.
- **Correction from the user:** Rescore from main, not the old worktree. Any credible current scratch evidence must earn some score, and menu turnover must not count toward scratch production.
- **What required interpretation:** A legacy scalar S cannot be safely converted when its turnover contribution is inseparable. Nineteen calibrated scalar-only rows therefore moved to a partial-evidence tier rather than inheriting invalid complete scores. Partial scores report earned/observed-possible and coverage and are never normalized into G. Generic fresh claims, menu nouns, historical facts, beverage-only work and explicitly rejected customer-only inferences still do not establish current restaurant-food production.
- **What the SKILL or tooling still needed:** The canonical interactive decision schema had no representation for partial scores despite the v8.14 prose contract. It was extended to permit null S/I/G with partial earned points, observed possible points, coverage, confidence and criterion fields. A run-local deterministic validator now checks population, unique IDs, criterion bounds, complete sums, partial non-normalization and G arithmetic.
- **Verification:** 1,650 records and 1,650 unique IDs; 207 complete new-basis scores; 444 partial scores; 906 evidence-exhausted/no-score; 33 positive DQs; 55 reversible familiarity exclusions; five canonical duplicates. Validator reports zero errors.
- **Progress:** Canonical v8.14 JSON, validation report and regenerated 680-line reader audit are complete in the new main-checkout run directory. The legacy decision ledgers remain preserved alongside them.
