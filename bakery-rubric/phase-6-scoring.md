...
- **Per-number provenance remains mandatory.** Treat every S, I, E, R, and price value as direct-documented, provisional-secondary, estimated, or unverified. Reader-facing rated tiers require an accepted direct or provisional rating and an S decision grounded in accepted evidence.
- **A valid provisional rating may clear the existing rating floor.** Use one only after Phase 5 accepts the complete `aggregator-attributed` contract and required direct-place attempts. It participates in normal scoring and occasion ranking, but it cannot break an existing near-tie against a directly observed rating. A later valid direct observation replaces it.
- **Rating exhaustion is terminal, not negative.** A scratch-verified bakery whose rating is `exhausted-unavailable` after the required search trail and has no accepted provisional rating goes to the explicit **scratch-verified, rating-unconfirmed** tier. It is surfaced with that caveat, neither silently dropped nor promoted into a rating-gated tier. Estimated or unverified ratings remain non-terminal.
- **Conflicts remain visible.** Preserve each literal rating, count, source, provenance type, and date; prefer a current identity-matched direct source over aggregator-attributed evidence, flag material discrepancies, and never synthesize a rating.
...
- [ ] Every provisional rating satisfies the Phase 5 aggregator-attributed contract, remains provenance-labeled, and was not used to break a near-tie against a direct rating.
